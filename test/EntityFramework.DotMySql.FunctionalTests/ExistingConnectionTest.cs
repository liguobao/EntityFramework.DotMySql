...
namespace Microsoft.Data.Entity.SqlServer.FunctionalTests
{
    public class ExistingConnectionTest
    {
        ...
        private static async Task Can_use_an_existing_closed_connection_test(bool openConnection)
        {
            ...
            using (var store = MySqlNorthwindContext.GetSharedStore())
            {
                var openCount = 0;
                var closeCount = 0;
                var disposeCount = 0;

                using (var connection = new MySqlConnection(store.Connection.ConnectionString))
                {
                    if (openConnection)
                    {
        ...
        private class NorthwindContext : DbContext
        {
            ...
            public NorthwindContext(IServiceProvider serviceProvider, MySqlConnection connection)
