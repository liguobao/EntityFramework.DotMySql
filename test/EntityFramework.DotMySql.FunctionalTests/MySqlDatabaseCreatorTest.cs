    ...
    public class MySqlDatabaseCreatorTest
    {
        ...
        private static async Task HasTables_throws_when_database_doesnt_exist_test(bool async)
        {
            using (var testDatabase = await MySqlTestStore.CreateScratchAsync(createDatabase: false))
            {
                ...
                var errorNumber = async
                    ? (await Assert.ThrowsAsync<MySqlException>(() => ((TestDatabaseCreator)creator).HasTablesAsyncBase())).Number
                    : Assert.Throws<MySqlException>(() => ((TestDatabaseCreator)creator).HasTablesBase()).Number;
        ...
        private static async Task Delete_throws_when_database_doesnt_exist_test(bool async)
        {
            using (var testDatabase = await MySqlTestStore.CreateScratchAsync(createDatabase: false))
            {
                ...
                if (async)
                {
                    await Assert.ThrowsAsync<MySqlException>(() => creator.DeleteAsync());
                }
                else
                {
        ...
        private static async Task CreateTables_creates_schema_in_existing_database_test(bool async)
        {
            using (var testDatabase = await MySqlTestStore.CreateScratchAsync())
            {
                ...
                optionsBuilder.UseMySql(testDatabase.Connection.ConnectionString);
        ...
        private static async Task CreateTables_throws_if_database_does_not_exist_test(bool async)
        {
            using (var testDatabase = await MySqlTestStore.CreateScratchAsync(createDatabase: false))
            {
                ...
                var errorNumber
                    = async
                        ? (await Assert.ThrowsAsync<MySqlException>(() => creator.CreateTablesAsync())).Number
                        : Assert.Throws<MySqlException>(() => creator.CreateTables()).Number;
        ...
        private static IServiceProvider CreateContextServices(MySqlTestStore testStore)
        {
            var serviceCollection = new ServiceCollection();
            serviceCollection
                .AddEntityFramework()
            ...
            serviceCollection.AddScoped<MySqlDatabaseCreator, TestDatabaseCreator>();
        ...
        private static IRelationalDatabaseCreator GetDatabaseCreator(MySqlTestStore testStore)
        ...
        public class TestDatabaseCreator : MySqlDatabaseCreator
        {
            public TestDatabaseCreator(
                ...
                IMigrationsSqlGenerator SqlGenerationHelper,
                IModel model,
                IRawSqlCommandBuilder sqlCommandBuilder)
                : base(connection, modelDiffer, SqlGenerationHelper, sqlCommandBuilder, model)
            {
            }
