...
WHERE `e`.`ReportsTo` IS NULL",
        ...
        public override void Select_collection_navigation_simple()
        {
            ...
            Assert.Equal(
                @"SELECT `c`.`CustomerID`
FROM `Customers` AS `c`
WHERE `c`.`CustomerID` LIKE CONCAT('A','%')
...
FROM `Orders` AS `o`
WHERE `o`.`CustomerID` LIKE CONCAT('A','%')",
                Sql);
        }
...
";
        ...
        public QueryNavigationsMySqlTest(NorthwindQueryMySqlFixture fixture, ITestOutputHelper testOutputHelper)
            : base(fixture)
        {
